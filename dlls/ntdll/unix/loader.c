...
#define SYSCALL_STUB(name) \
    static void name(void) { stub_syscall(#name); }
ALL_SYSCALL_STUBS
...
static char *remove_tail(const char *str, const char *tail)
{
    ...
    if (len < tail_len)
        return NULL;
    if (strcmp(str + len - tail_len, tail))
        return NULL;
...
static char *build_relative_path(const char *base, const char *from, const char *dest)
{
    ...
    for (;;)
    {
        ...
        while (*from && *from != '/' && *from == *dest)
        {
            from++;
            dest++;
        }
        if ((!*from || *from == '/') && (!*dest || *dest == '/'))
            continue;
...
/* CW Hack 24067 */
__attribute__((visibility("default"))) void prepend_dll_path(const char *path)
{
...
static void set_config_dir(void)
{
    ...
    else
    {
        ...
        if (home_dir[0] != '/')
            fatal_error("the home directory %s is not an absolute path\n", home_dir);
...
static void init_paths(void)
{
    ...
    {
        if (!(dll_dir = remove_tail(ntdll_dir, get_so_dir(current_machine))))
            dll_dir = ntdll_dir;
...
char *get_alternate_wineloader(WORD machine)
{
    ...
    if (is_win64)
    {
        if (force_wow64)
            return NULL;
        if (machine != get_alt_machine(current_machine))
            return NULL;
    }
    else
    {
        if (!force_wow64 && machine == current_machine)
            return NULL;
...
static NTSTATUS loader_exec(char **argv, WORD machine, const char *image_path)
{
    ...
    if (((argv[1] = get_alternate_wineloader(machine))))
...
static int exec_wineserver(pid_t *pid, char **argv)
{
    ...
    if (!build_path_and_exec(pid, bin_dir, "wineserver", argv))
        return 0;
    if ((path = getenv("WINESERVER")) && !build_path_and_exec(pid, "", path, argv))
        return 0;
...
void start_server(BOOL debug)
{
    ...
    if (!started)
    {
        ...
        if (status == 2)
            return; /* server lock held by someone else, will retry later */
        if (status)
            exit(status); /* server failed */
        started = TRUE;
    }
}
...
{
    if (index >= ARRAY_SIZE(KeServiceDescriptorTable))
        return FALSE;
...
static void fixup_so_resources(IMAGE_RESOURCE_DIRECTORY *dir, BYTE *root, int delta)
{
    ...
    for (i = 0; i < dir->NumberOfNamedEntries + dir->NumberOfIdEntries; i++, entry++)
    {
        ...
        if (entry->DataIsDirectory)
            fixup_so_resources(ptr, root, delta);
        else
...
static NTSTATUS map_so_dll(const IMAGE_NT_HEADERS *nt_descr, HMODULE module)
{
    ...
    code_start = (sizeof(IMAGE_DOS_HEADER) + sizeof(builtin_signature) + sizeof(IMAGE_NT_HEADERS) + nb_sections * sizeof(IMAGE_SECTION_HEADER) + align_mask) & ~align_mask;

    if (anon_mmap_fixed(addr, code_start, PROT_READ | PROT_WRITE, 0) != addr)
...
static NTSTATUS load_so_dll(void *args)
{
    ...
    if (get_load_order(nt_name) == LO_DISABLED)
    ...
    if (!get_nt_and_unix_names(&attr, &true_nt_name, &unix_name, FILE_OPEN, FALSE))
    {
        ...
        if (len > 3 && !wcsicmp(nt_name->Buffer + len - 3, soW))
...
static int check_library_arch(int fd)
{
    ...
    if (sizeof(void *) == sizeof(int))
        return !(header.cputype >> 24);
    else
        return (header.cputype >> 24) == 1; /* CPU_ARCH_ABI64 */
#else
    struct /* ELF header */
    {
    ...
    if (read(fd, &header, sizeof(header)) != sizeof(header))
        return 1;
    if (memcmp(header.magic, "\177ELF", 4))
        return 1;
    if (header.version != 1 /* EV_CURRENT */)
        return 1;
#ifdef WORDS_BIGENDIAN
    if (header.data != 2 /* ELFDATA2MSB */)
        return 1;
#else
    if (header.data != 1 /* ELFDATA2LSB */)
        return 1;
#endif
    if (sizeof(void *) == sizeof(int))
        return header.class == 1; /* ELFCLASS32 */
    else
        return header.class == 2; /* ELFCLASS64 */
#endif
}
                                     ...
                                     USHORT load_machine, BOOL prefer_native)
{
    ...
    if (search_machine != current_machine)
        return status;
    if (load_machine && load_machine != current_machine)
        return status;
                                        ...
                                        SECTION_IMAGE_INFORMATION *image_info)
{
    ...
    if (!status)
        virtual_fill_image_information(&pe_info, image_info);
    return status;
}
...
static void hacks_init(void)
{
    ...
    if (env_str)
        simulate_writecopy = atoi(env_str);
                                           ...
                                           const char *top_dir, const char *build_dir)
{
    ...
    if (namelen > extlen && !strcmp(name + namelen - extlen, ext))
        namelen -= extlen;
...
static NTSTATUS open_dll_file(const char *name, OBJECT_ATTRIBUTES *attr, HANDLE *mapping)
{
    ...
    size.QuadPart = 0;
    status = NtCreateSection(mapping, STANDARD_RIGHTS_REQUIRED | SECTION_QUERY | SECTION_MAP_READ | SECTION_MAP_EXECUTE,
                                 ...
                                 USHORT load_machine, BOOL prefer_native, off_t offset)
{
    ...
    if (!exp_name || !exp_name->Length)
    {
        ...
        for (i = 0; i < len; i++)
            if (nt_name->Buffer[i] == '/' || nt_name->Buffer[i] == '\\')
                namepos = i + 1;
        len -= namepos;
        if (!len)
            return STATUS_DLL_NOT_FOUND;
    ...
    if (!exp_name || !exp_name->Length)
    {
        ...
        for (i = 0; i < len; i++)
        {
            if (nt_name->Buffer[namepos + i] > 127)
                goto done;
            file[pos + i] = (char)nt_name->Buffer[namepos + i];
        }
    }
    else
        memcpy(file + pos, exp_name->Buffer, len);

    for (i = 0; i < len; i++)
    {
        if (file[pos + i] >= 'A' && file[pos + i] <= 'Z')
            file[pos + i] += 'a' - 'A';
        else if (file[pos + i] == '.')
            ext = file + pos + i;
    }
                      ...
                      off_t offset)
{
    ...
    if (loadorder == LO_DISABLED)
        return STATUS_DLL_NOT_FOUND;

    if (image_info->wine_builtin)
    {
        if (loadorder == LO_NATIVE)
            return STATUS_DLL_NOT_FOUND;
...
static const WCHAR *get_machine_wow64_dir(WORD machine)
{
    ...
    if (machine == native_machine || wow64_using_32bit_prefix)
...
BOOL is_builtin_path(const UNICODE_STRING *path, WORD *machine)
{
    ...
    for (i = 0; i < supported_machines_count; i++)
    {
        ...
        if (len <= dirlen)
            continue;
        if (wcsnicmp(p, sysdir, dirlen))
            continue;
        /* check for remaining path components */
        for (p += dirlen, len -= dirlen; len; p++, len--)
            if (*p == '\\')
                return FALSE;
...
static void redirect_ntdll_functions(HMODULE module)
{
    ...
    if (!(loadcfg = get_module_data_dir(module, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, NULL)))
        return;
    if (!(metadata = (void *)loadcfg->CHPEMetadataPointer))
        return;
...
static void load_ntdll(void)
{
    ...
    if (build_dir)
        asprintf(&name, "%s%s/ntdll.dll", ntdll_dir, pe_dir);
    else
        asprintf(&name, "%s%s/ntdll.dll", dll_dir, pe_dir);
    ...
    if (status == STATUS_IMAGE_NOT_AT_BASE)
        status = virtual_relocate_module(module);
    if (status)
...
static void load_apiset_dll(void)
{
    ...
    if (build_dir)
        asprintf(&name, "%s/dlls/apisetschema%s/apisetschema.dll", build_dir, pe_dir);
    else
        asprintf(&name, "%s%s/apisetschema.dll", dll_dir, pe_dir);
    ...
    if (!status)
    {
        ...
        {
            if (memcmp((char *)sec->Name, ".apiset", 8))
                continue;
                ...
                map->Size <= sec->Misc.VirtualSize)
            {
                peb->ApiSetMap = map;
                if (wow_peb)
                    wow_peb->ApiSetMap = PtrToUlong(map);
...
static void load_wow64_ntdll(USHORT machine)
{
    ...
    if (machine == current_machine)
        return;
    if (!(wow64_dir = get_machine_wow64_dir(machine)))
        return;
...
static ULONG_PTR get_image_address(void)
{
    ...
    while (num--)
    {
        if (phdr->p_type == PT_PHDR)
            return phdr_addr - phdr->p_offset;
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    if ((*vm)->AttachCurrentThread(vm, &env, NULL) != JNI_OK)
        return JNI_ERR;
    if (!(class = (*env)->FindClass(env, WINE_JAVA_CLASS)))
        return JNI_ERR;
...
static void apple_create_wine_thread(void *arg)
{
    ...
    if (pthread_create(&thread, &attr, apple_wine_thread, NULL))
        exit(1);
...
static int pre_exec(void)
{
    if (build_dir)
    {
        ...
        if (path)
            asprintf(&path, "%s/dlls/ntdll:%s/dlls/win32u:%s", build_dir, build_dir, path);
        else
            asprintf(&path, "%s/dlls/ntdll:%s/dlls/win32u", build_dir, build_dir);
...
static void reexec_loader(int argc, char *argv[], char *extra_arg)
{
    ...
    if (!pre_exec() && !extra_arg && dlsym(RTLD_DEFAULT, "wine_main_preload_info"))
        return;
...
static void check_command_line(int argc, char *argv[])
{
    ...
    if (strcmp(basename, "wine")) /* check if there's a builtin exe corresponding to the base name */
    {
        ...
        if (build_dir)
        {
            ...
            if (!access(exe, R_OK))
                reexec_loader(argc, argv, basename);
            free(exe);
        }
        else
        {
...
DECLSPEC_EXPORT void __wine_main(int argc, char *argv[])
{
    ...
    init_paths();
    if (!getenv("WINELOADERNOEXEC") || argc <= 1)
