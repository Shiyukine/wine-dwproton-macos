...
static inline void set_context_exception_reporting_flags(DWORD *context_flags, DWORD reporting_flag)
{
    ...
    *context_flags = (*context_flags & ~(CONTEXT_SERVICE_ACTIVE | CONTEXT_EXCEPTION_ACTIVE)) | CONTEXT_EXCEPTION_REPORTING | reporting_flag;
}
...
{
    if (io && in_wow64_call())
        return wine_server_client_ptr(io->Pointer);
...
{
    if ((bits.granularity = (limit >= 0x100000)))
        limit >>= 12;
    bits.limit = limit;
    return bits;
}
