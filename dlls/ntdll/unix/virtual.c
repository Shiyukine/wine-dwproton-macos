...
static void kernel_writewatch_register_range(struct file_view *view, void *base, size_t size)
{
    ...
    if (!(view->protect & VPROT_WRITEWATCH) || !use_kernel_writewatch)
        return;
...
#endif
    for (i = 0; i < host_page_size / page_size; i++)
        vprot |= vprot_ptr[i];
    return vprot;
}
    ...
    if (top_down)
    {
        ...
        do
        {
            if (start >= end || start < base || (char *)end - (char *)start < size)
                return NULL;
            if (start < range->end && start >= range->base && (char *)range->end - (char *)start >= size)
                break;
...
static int mprotect_range(void *base, size_t size, BYTE set, BYTE clear)
{
    ...
    for (count = i = 1; i < size / host_page_size; i++, count++)
    {
        ...
        if (next == prot)
            continue;
        if (mprotect_exec(addr, count * host_page_size, prot))
            return -1;
    ...
    if (top_down)
    {
        LIST_FOR_EACH_ENTRY_REV(area, &reserved_areas, struct reserved_area, entry)
        {
            ...
            if (end <= limit_low)
                return NULL;
            if (start < limit_low)
                start = (void *)ROUND_SIZE(0, limit_low, host_page_mask);
            if (end > limit_high)
                end = ROUND_ADDR(limit_high, host_page_mask);
...
static NTSTATUS map_fixed_area(void *base, size_t size, int unix_prot)
{
    ...
    LIST_FOR_EACH_ENTRY(area, &reserved_areas, struct reserved_area, entry)
    {
        ...
        if (area_start > start)
        {
            if (anon_mmap_tryfixed(start, area_start - start, unix_prot, 0) == MAP_FAILED)
                goto failed;
            start = area_start;
        }
        if (area_end >= end)
                         ...
                         ULONG_PTR limit_low, ULONG_PTR limit_high, size_t align_mask)
{
    ...
    if (base)
    {
        ...
        if (is_beyond_limit(base, size, host_addr_space_limit))
            return STATUS_CONFLICTING_ADDRESSES;
        if ((status = map_fixed_area(base, size, unix_prot)))
            return status;
                                   ...
                                   off_t offset, unsigned int vprot, BOOL removable)
{
    ...
    if (map_addr + map_size >= (char *)view->base + view->size)
        host_size = map_size;
    else
        host_size = ROUND_SIZE(0, map_size, host_page_mask);
...
static NTSTATUS decommit_pages(struct file_view *view, char *base, size_t size)
{
    ...
    if (host_start < host_end)
        anon_mmap_fixed(host_start, host_end - host_start, PROT_NONE, 0);
...
static NTSTATUS map_pe_header(void *ptr, size_t size, size_t map_size, int fd, BOOL *removable)
{
    ...
    if (!*removable && map_size)
    {
        ...
        {
            if (size > map_size)
                pread(fd, (char *)ptr + map_size, size - map_size, map_size);
            return STATUS_SUCCESS;
        }
        switch (errno)
        {
    ...
    for (i = pos = 0; i < nt->FileHeader.NumberOfSections; i++)
    {
            ...
            (sec[i].Characteristics & IMAGE_SCN_MEM_WRITE))
        {
                ...
                imports->VirtualAddress < sec[i].VirtualAddress + map_size)
            {
                ...
                if (end > sec[i].VirtualAddress + map_size)
                    end = sec[i].VirtualAddress + map_size;
                if (end > base)
                                        ...
                                        ULONG alloc_type, ULONG protect, USHORT machine)
{
    ...
    if (!(size = ROUND_SIZE(0, size, page_mask)))
...
{
#if defined(HAVE_SYSINFO) && defined(HAVE_STRUCT_SYSINFO_TOTALRAM) && defined(HAVE_STRUCT_SYSINFO_MEM_UNIT)
                                     ...
                                     struct pe_image_info *info, void *so_handle)
{
    ...
    server_enter_uninterrupted_section(&virtual_mutex, &sigset);
    status = create_view(&view, base, size, SEC_IMAGE | SEC_FILE | VPROT_SYSTEM | VPROT_COMMITTED | VPROT_READ | VPROT_WRITECOPY | VPROT_EXEC);
    if (!status)
    {
...
WORD ldt_update_entry(WORD sel, LDT_ENTRY entry)
{
    ...
    if (!ldt_copy)
    {
        ...
        if (map_view(&view, NULL, sizeof(*ldt_copy), MEM_TOP_DOWN,
                     ...
                     is_win64 ? limit_2g : 0, limit_4g, 0))
            return 0;
        ldt_copy = view->base;
        if (is_win64)
            wow_peb->SpareUlongs[0] = PtrToUlong(ldt_copy);
        else
            peb->SpareUlongs[0] = PtrToUlong(ldt_copy);
    }
...
NTSTATUS ldt_get_entry(WORD sel, CLIENT_ID client_id, LDT_ENTRY *entry)
{
    ...
    else
    {
        ...
        if ((status = NtOpenProcess(&process, PROCESS_ALL_ACCESS, NULL, &client_id)))
            return status;
        ...
        if (!is_win64)
        {
            ...
            NtQueryInformationProcess(process, ProcessWow64Information, &peb32, sizeof(peb32), NULL);
    ...
    if (base || bits.limit || bits.type)
        *entry = ldt_make_entry(base, bits);
    else
...
NTSTATUS WINAPI NtSetLdtEntries(ULONG sel1, LDT_ENTRY entry1, ULONG sel2, LDT_ENTRY entry2)
{
    ...
    server_enter_uninterrupted_section(&virtual_mutex, &sigset);
    if (sel1)
        ldt_update_entry(sel1, entry1);
    if (sel2)
        ldt_update_entry(sel2, entry2);
    server_leave_uninterrupted_section(&virtual_mutex, &sigset);
    return STATUS_SUCCESS;
}
                                    ...
                                    SIZE_T reserve_size, SIZE_T commit_size, BOOL guard_page)
{
    ...
    /* ROSETTA 2 HACK: Significantly boost the stack size.
     * Rosetta 2 emulation introduces noticeable stack overhead per frame.
     * Combined with removing the guard page, we need a massive runway
     * so the stack never overruns the allocated view boundary. */
    if (size < 8 * 1024 * 1024)
        size = 8 * 1024 * 1024; /* Force minimum 8MB stack */
    size = size * 2;            /* Double the requested size for extra safety padding */
    ...
    /* HACK: skip guard-page setup (Kept from your previous fix) */
    VIRTUAL_DEBUG_DUMP_VIEW(view);
...
void virtual_init_user_shared_data(void)
{
    ...
    case IMAGE_FILE_MACHINE_I386:
        data->NativeProcessorArchitecture = PROCESSOR_ARCHITECTURE_INTEL;
        break;
    case IMAGE_FILE_MACHINE_AMD64:
        data->NativeProcessorArchitecture = PROCESSOR_ARCHITECTURE_AMD64;
        break;
    case IMAGE_FILE_MACHINE_ARMNT:
        data->NativeProcessorArchitecture = PROCESSOR_ARCHITECTURE_ARM;
        break;
...
ssize_t virtual_locked_recvmsg(int fd, struct msghdr *hdr, int flags)
{
    ...
    if (has_write_watch)
        while (i--)
            update_write_watches(hdr->msg_iov[i].iov_base, hdr->msg_iov[i].iov_len, 0);
...
static BOOL force_laa(void)
{
    ...
    if ((app_name = ntdll_wcsrchr(main_wargv[0], '\\')))
        app_name++;
    else
        app_name = main_wargv[0];
                                        ...
                                        ULONG_PTR align, ULONG attributes)
{
    ...
    if (!(type & (MEM_COMMIT | MEM_RESERVE | MEM_RESET)) || (type & MEM_REPLACE_PLACEHOLDER && !(type & MEM_RESERVE)))
    {
...
NTSTATUS WINAPI NtAllocateVirtualMemoryEx(HANDLE process, PVOID *ret, SIZE_T *size_ptr, ULONG type,
                                          ...
                                          ULONG count)
{
    static const ULONG type_mask = MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN | MEM_WRITE_WATCH | MEM_RESET | MEM_RESERVE_PLACEHOLDER | MEM_REPLACE_PLACEHOLDER;
                                       ...
                                       ULONG new_prot, ULONG *old_prot)
{
    ...
    if ((view = find_view(base, size)))
    {
        ...
        if (get_committed_size(view, base, size, &vprot, VPROT_COMMITTED) >= size && (vprot & VPROT_COMMITTED))
        {
            ...
            if (simulate_writecopy && status == STATUS_SUCCESS && ((old == PAGE_WRITECOPY || old == PAGE_EXECUTE_WRITECOPY)))
            {
    ...
    *region_end = working_set_limit;
...
#ifdef __i386__
    {
        ...
        LIST_FOR_EACH_ENTRY(area, &reserved_areas, struct reserved_area, entry)
        {
            ...
            if (area_start < *region_end)
                *region_end = area_start;
            break;
        }
        *fake_reserved = TRUE;
    }
...
static void init_fill_working_set_info_data(struct fill_working_set_info_data *d, char *end)
{
    ...
    if (pagemap_fd == -1)
        ...
        fcntl(pagemap_fd, F_SETFD, FD_CLOEXEC); /* in case O_CLOEXEC isn't supported */
}
                                   ...
                                   SIZE_T len, SIZE_T *res_len)
{
    ...
    while (view && (char *)view->base < end)
    {
        ...
        while (r != ref + count && r->addr < start)
            ++r;
        while (start != (char *)view->base + view->size && r != ref + count && r->addr < (char *)view->base + view->size)
        {
                                   ...
                                   SECTION_INHERIT inherit, ULONG alloc_type, ULONG protect)
{
    ...
    else if (alloc_type & AT_ROUND_TO_PAGE)
        return STATUS_INVALID_PARAMETER_9;

    if (alloc_type & MEM_REPLACE_PLACEHOLDER)
        mask = page_mask;
    if (offset.u.LowPart & mask)
        return STATUS_MAPPED_ALIGNMENT;
    if ((UINT_PTR)*addr_ptr & mask)
        return STATUS_MAPPED_ALIGNMENT;
