...
static const WCHAR *hack_append_command_line(const WCHAR *cmd, const WCHAR *cmd_line)
{
    ...
    {
        if (wcsstr(cmd, options[i].exe_name) && (!options[i].required_args || wcsstr(cmd_line, options[i].required_args)) && (!options[i].forbidden_args || !wcsstr(cmd_line, options[i].forbidden_args)))
        {
